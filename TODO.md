...
- [ ] VAL-10 | P2 | LOW | orchestrator | One-off cleanup of orphan auth users (auth.users rows whose `public.users` mirror was wiped by `seed --reset`). List via `supabase.auth.admin.listUsers`, filter non-seed, INSERT mirror or delete the orphan auth user. Document in `docs/SUPABASE_SETUP.md`.
- [x] VAL-11 | P1 | **HIGH** | auth-agent (followup) | **`createFirstOrgAction` self-heal.** `memberships.user_id → users.id` FK fails if `public.users` mirror was wiped (e.g. dev `seed --reset` cascade leaves auth.users orphaned). Fix in `services/orgs-service.ts:createOrgWithAdmin` (or the repo): `INSERT INTO users (id, email) ... ON CONFLICT (id) DO NOTHING` from current Supabase user before the membership write. Single transaction. Add unit test that simulates orphan auth.users + asserts org create succeeds.
- [ ] VAL-12 | P2 | LOW | seed-agent (followup) | `seed --reset` should also drop matching Supabase auth users to keep `auth.users` and `public.users` in sync. Otherwise dev cycles leave landmines like VAL-11. Use `auth.admin.deleteUser(id)` for any auth user with email matching `*@seed.contrario.dev` before truncate.
- [x] VAL-13 | P1 | MEDIUM | auth-agent (followup) | **Onboarding layout missing nav + logout.** `/onboarding/create-org` shows only the form on a blank shell. Authenticated user has no way to sign out / abandon / get help if stuck (e.g. FK error from VAL-11). Add minimal top bar to `app/onboarding/layout.tsx` with: Logo, signed-in-as email/avatar, **Sign out** action. No side nav (still pre-org), but escape hatches present. Same `AuthCard`-tier visual.

### Walkthrough findings (Railway deploy, 2026-05-11)

- [ ] VAL-14 | P1 | MEDIUM | auth-agent (followup) | **Org switcher not data-driven.** `components/app-shell/org-switcher-slot.tsx` is hardcoded to render a `Create organization` Link with a `TODO(auth-agent)` marker. User has multiple memberships (created via `/onboarding/create-org` twice) but cannot see active org name, list orgs, or switch. Backend already exists (`services.orgs.listForCurrentUser`, `switchOrgAndRedirect` in `features/orgs/server/orgs-actions.ts`). Convert to async server component: fetch `getRequestContext` + `listForCurrentUser`, render shadcn `DropdownMenu` listing all orgs with active marked by `Check` icon, click = `<form action={switchOrgAndRedirect}>` with `orgId` hidden input. Keep "Create organization" item at bottom. Fallback to existing CTA when zero memberships. Acceptance: pick a non-active org → cookie rewrites → `revalidatePath('/', 'layout')` runs → next render shows new org name in trigger.
- [ ] VAL-15 | P1 | MEDIUM | notes-agent (followup) | **Newly created notes don't appear in `/` recent notes section.** User creates a note via `/notes/new` → success → returns to `/` → "No recent notes yet" empty state still shown. Investigation: home page (`app/(app)/page.tsx`) likely doesn't fetch `services.notes.listVisible` at all OR fetches but the RSC tree is cached. Could be: (a) home is static and doesn't read DB, (b) home reads DB but missing `revalidatePath('/')` in `createNote` action, (c) limit is wrong. Reproduce against cloud: sign in, create note, navigate `/notes/<id>` (should show note → confirms DB write succeeded), navigate `/` → should now show note. Fix in `app/(app)/page.tsx` if not wired, or in `features/notes/server/notes-actions.ts:createNote` to call `revalidatePath('/')` after successful insert.
- [ ] VAL-16 | P1 | LOW | auth-agent (followup) OR frontend-builder (followup) | **Profile dropdown elements disabled.** Top-bar avatar opens an `Account` dropdown with `Settings` + `Sign out` items both visually disabled (grayed out, no hover). Either: (a) Settings link not wired to `/settings` route (placeholder), or (b) Sign out form not wired to `signOutAction`. Verify `components/app-shell/user-menu-slot.tsx`. Fix: Settings → `<Link href="/settings">` (route exists as placeholder), Sign out → `<form action={signOutAction}>` with submit button (same pattern as `OnboardingTopBar`).
- [x] VAL-18 | P1 | MEDIUM | auth-agent (followup) | **Invite by email.** `/settings/members` admin-only invite form. `services.orgs.inviteByEmail` resolves existing emails → membership insert + JWT sync; new emails → `admin.auth.admin.inviteUserByEmail` with `invited_org_id`/`invited_role` in `user_metadata`. Accept-invite landing at `/onboarding/accept-invite` (hit via `/auth/callback` after the email link) reads the metadata, inserts the membership idempotently, syncs the JWT, writes the active-org cookie, and clears the `invited_*` keys. PR: TBD.
